...
from util.natural_wrapper import ReplaceBackgroundEnv, BackgroundMattingWithColor, NoiseSource, VideoSource
import cv2
...
class Agent:
    def __init__(self, hyperparameter_set):
        ...
        # Maximum steps before stopping training (None for no limit)
        self.max_steps = hyperparameters.get('max_steps', None)

        # Background replacement settings
        self.background_type = hyperparameters.get('background_type', 'none') # 'none', 'noise', 'video', 'kinetics'
        self.video_path = hyperparameters.get('video_path', None)
        self.videos_folder = hyperparameters.get('videos_folder', 'videos')
    ...
    def _get_random_video_path(self):
        """Randomly select a video file from the videos folder."""
        video_pattern = os.path.join(self.videos_folder, '**', '*.mp4')
        video_files = glob.glob(video_pattern, recursive=True)
        
        if not video_files:
            raise ValueError(f"No MP4 files found in {self.videos_folder} folder")
    ...
    def run(self, is_training=True, render=False):
        ...
        render_mode = None
        if render:
            # If background replacement is active, we need 'rgb_array' to modify the frame before showing it
        ...
        if self.background_type != 'none':
            # Apply background replacement wrapper
            # Breakout resolution is (210, 160, 3)
            bg_matting = BackgroundMattingWithColor(color=[0, 0, 0], threshold=10)
            
            if self.background_type == 'noise':
                source = NoiseSource(shape=(210, 160), strength=255)
            elif self.background_type == 'video':
                if self.video_path is None:
                    raise ValueError("video_path must be specified when background_type is 'video'")
                source = VideoSource(shape=(210, 160), file_path=self.video_path)
            elif self.background_type == 'kinetics':
                random_video_path = self._get_random_video_path()
                source = VideoSource(shape=(210, 160), file_path=random_video_path, videos_folder=self.videos_folder)
            else:
                raise ValueError(f"Unknown background_type: {self.background_type}")
                
            env = ReplaceBackgroundEnv(env, bg_matting, source)
        ...
        for episode in itertools.count():
            ...
            while (not terminated and episode_reward < self.stop_on_reward):
                ...
                if is_training and self.max_steps is not None and total_steps >= self.max_steps:
                ...
                # Manual rendering for background replacement
                if render and self.background_type != 'none':
                    frame = env.render()
                    if frame is not None:
                        # Convert RGB to BGR for OpenCV
                        frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                        cv2.imshow('Breakout', frame_bgr)
                        cv2.waitKey(1)
            ...
            if is_training:
                ...
                # Save best score model (Primary - most important)
                if episode_reward > best_reward:
                    best_reward = episode_reward
                    torch.save(policy_dqn.state_dict(), self.MODEL_FILE)
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: New best score {best_reward:0.1f} at episode {episode}, saving model..."
                    print(log_message)
